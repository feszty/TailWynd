...
- `images/AppIcon.png` is the Tailwynd app icon (also used as favicon
  and nav logo).
- `Tailwynd-Beta-Field-Guide.pdf` (site root) is linked from the hero
  buttons on index.html and from the "Try the beta" section on
  support.html — keep the filename if you regenerate the PDF, or
  update both links.
